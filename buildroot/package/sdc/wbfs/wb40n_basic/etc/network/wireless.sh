...
wifi_start()
{
  grep -q dhd /proc/modules && {
    ...
    then
      ...
      dhd -i $WIFI_DEV download $WIFI_FIRMWARE $WIFI_NVRAM || exit 1
    fi
  }  
...
#
# WARNING:
# deconfig of iface - disabled per request
# termination of apps using iface - disabled per request
#
wifi_stop() {
  if [ -n "$WIFI_DEV" ] \
...
#[ "5" != "`grep -cEs '_PREFIX=|_DEVICE=|_DRIVER=|_LOG=|_TIMEOUT=' $WIFI_CONFIG`" ]
if [ ! -f $WIFI_CONFIG ]
then
